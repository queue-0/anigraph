...
function initGraph() {
  ...
  graphInstance = ForceGraph()(wrap)
    ...
    // d3Force is available via the force-graph library
    .d3Force('charge', null)   // we'll set it after
    .d3Force('link', null)     // reset, will configure below
    .d3Force('collision', null);

  // Apply custom forces after the graph is initialized
  const sim = graphInstance.d3Force;

  // Charge: strong repulsion to spread nodes out
  graphInstance.d3Force('charge',
    // Use d3 directly if available, or via the force-graph d3 re-export
    (typeof d3 !== 'undefined' ? d3 : ForceGraph.d3)
      .forceManyBody()
      .strength(-120)
      .distanceMax(600)
  );

  // Collision: add extra padding around meta nodes
  graphInstance.d3Force('collision',
    (typeof d3 !== 'undefined' ? d3 : ForceGraph.d3)
      .forceCollide(n => {
        // Meta nodes get a bigger collision radius so they don't crowd
