 ...
 *
 * Performance:
 *  - Uses dagre/d3-force precomputed layout then freezes nodes (no dragging)
 *  - Simulation runs warmupTicks then freezes all node positions
 *  - Node dragging disabled after layout
 */
...
let highlightedClusterIds = null;  // Set of node ids in highlighted cluster
let highlightedChainIds   = null;  // Set of node ids in highlighted chain

// ── CONSTANTS ─────────────────────────────────────────────────────────────────
const COLLISION_NODE_THRESHOLD = 5000;
...
function initGraph() {
  ...
  graphInstance
    ...
    .enableNodeDrag(false)  // disabled — positions are precomputed
    // Fast settle then freeze
    .d3AlphaDecay(0.04)
    .d3VelocityDecay(0.5)
    .warmupTicks(80)
    .cooldownTicks(0)    // freeze immediately after warmup
    .cooldownTime(0);

  // Strong repulsion for clean spread
  const chargeForce = graphInstance.d3Force('charge');
  if (chargeForce?.strength) {
    chargeForce.strength(-120).distanceMax(500);
  }

  // No collision (expensive at scale)
  ...
  document.getElementById('graph-canvas').addEventListener('mousemove', e => {
    ...
    const x = e.clientX + 14 + 280 > vw ? e.clientX - 280 - 14 : e.clientX + 14;
    const y = Math.min(e.clientY - 10, vh - 240);
    tt.style.left = x + 'px';
    tt.style.top  = y + 'px';
  });
...
function renderGraph({ nodes, links, filteredAnime, highlightedAnime, visibleMeta }) {
  ...
  setTimeout(() => {
    const clusterData = calculateAnimeClusters(nodes, links);
    const { count, largest, largestClusterIds } = clusterData;
    ...
    const chainResult = calculateLongestChain(nodes, links);
    document.getElementById('stat-longest-chain').textContent = chainResult.length.toLocaleString();
    window._longestChainIds = chainResult.ids;
  }, 100);

  setTimeout(() => graphInstance.zoomToFit(700, 40), 600);
}
...
function highlightLargestCluster() {
  ...
  clearSelection();
  clearLegendHighlight();
  highlightedClusterIds = window._largestClusterIds;
...
function highlightLongestChain() {
  ...
  clearSelection();
  clearLegendHighlight();
  highlightedChainIds   = window._longestChainIds;
  highlightedClusterIds = null;
  refreshColors();
}
...
function resolveNodeColor(n) {
  ...
  // Cluster/chain highlight takes priority
  if (highlightedClusterIds || highlightedChainIds) {
    const activeSet = highlightedClusterIds || highlightedChainIds;
    return activeSet.has(n.id) ? base : dimColor(base, 0.08);
  }
...
function resolveLinkColor(l) {
  ...
  if (highlightedClusterIds || highlightedChainIds) {
    const activeSet = highlightedClusterIds || highlightedChainIds;
    if (!activeSet.has(src) && !activeSet.has(tgt)) return getLinkColor(l.kind, 0.03);
...
function resolveLinkWidth(l) {
  if (!selectedNodeId && !legendHighlightKey && !highlightedClusterIds && !highlightedChainIds) return 0.5;
  const src = typeof l.source === 'object' ? l.source.id : l.source;
  const tgt = typeof l.target === 'object' ? l.target.id : l.target;

  if (highlightedClusterIds || highlightedChainIds) {
    const activeSet = highlightedClusterIds || highlightedChainIds;
    return (activeSet.has(src) && activeSet.has(tgt)) ? 2 : 0.1;
  }
...
function handleNodeHover(node) {
  ...
  if (n.type === 'anime') {
    ...
    // Show genres (from genre_ids or tag_ids that match GENRE_TAGS)
    const genreNames = [
      ...(n.genre_ids || []).map(id => window._nodeById?.get(id)?.name),
      ...(n.tag_ids   || []).map(id => {
        const nd = window._nodeById?.get(id);
        return (nd && window.GENRE_TAGS?.has(nd.name)) ? nd.name : null;
      }),
    ].filter(Boolean).slice(0, 6);
    const tagNames = (n.tag_ids || [])
      .map(id => window._nodeById?.get(id)?.name)
      .filter(name => name && !window.GENRE_TAGS?.has(name))
      .slice(0, 5);
