...
function buildLegend(filteredAnime, colorBy, visibleNodeTypes, visibleMetaForLegend) {
  ...
  if (colorBy === 'node_type') {
    addItem('#e8a030', `Anime (${totalAnime.toLocaleString()})`);
    ...
    YEAR_BANDS.forEach(b => {
      if (bandCounts[b.label]) {
        addItem(`hsl(${b.hue},70%,55%)`, `${b.label} (${bandCounts[b.label].toLocaleString()})`);
      }
    });
  ...
  // Meta node legend entries with counts from visibleMeta
  const metaCounts = {};
  if (visibleMetaForLegend) {
    visibleMetaForLegend.forEach(n => {
      metaCounts[n.type] = (metaCounts[n.type] || 0) + 1;
    });
  }
  metaTypes.forEach(([type, label]) => {
    if (visibleNodeTypes && visibleNodeTypes.has(type)) {
      const cnt = metaCounts[type] || 0;
      addItem(META_NODE_COLORS[type], cnt > 0 ? `${label} (${cnt.toLocaleString()})` : label);
    }
  });
}
