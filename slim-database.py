...
# ── ANILIST HELPERS ───────────────────────────────────────────────────────────
# The browser-side anilist.js works because browsers send a real User-Agent.
# Python's urllib sends "Python-urllib/3.x", which Cloudflare blocks with 403.
# Fix: spoof a modern Chrome User-Agent, matching what the browser would send.
...
def anilist_query(query: str, variables: dict, retries: int = 4) -> dict | None:
    ...
    for attempt in range(retries):
        try:
            ...
            if e.code == 403:
                print("       Cloudflare is blocking — check User-Agent or try again later.")
                return None
            elif e.code == 429:
